...
async function startRecording() {
    ...
    try {
        ...
        function updateWaveform() {
            ...
            // Define frequency range weights to emphasize all frequencies, with higher sensitivity on the left
            const weights = [1.25, 1.2, 1.15, 1.1, 1.0];
            
            for (let i = 0; i < barCount; i++) {
                let sum = 0;
                // Adjust frequency ranges to capture more frequencies across all bars
                const startIdx = Math.floor((i / barCount) * dataArray.length * 0.6); // Use more of the frequency range
                const endIdx = Math.floor(((i + 1) / barCount) * dataArray.length * 0.6);
                ...
                // Increased overall sensitivity and maximum height
                const height = Math.max(5, (average / 255) * 180);
