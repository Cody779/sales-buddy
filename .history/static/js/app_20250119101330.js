...
async function startRecording() {
    ...
    try {
        ...
        function updateWaveform() {
            ...
            // Define frequency range weights to emphasize mid-range frequencies
            const weights = [0.8, 1.0, 1.2, 1.0, 0.8];
            
            for (let i = 0; i < barCount; i++) {
                let sum = 0;
                // Use weighted ranges to capture more of the voice frequencies
                const startIdx = Math.floor((i / barCount) * dataArray.length * 0.8); // Only use 80% of frequency range
                const endIdx = Math.floor(((i + 1) / barCount) * dataArray.length * 0.8);
                ...
                // Increased base height and adjusted multiplier for more dynamic movement
                const height = Math.max(5, (average / 255) * 150);
